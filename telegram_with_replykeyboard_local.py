...
def random_query(message):
    post_id = random.randrange(1, archillect_local.get_last_post_index() + 1)
    ii = archillect_local.get_image(post_id)
...
def random5_query(message):
    last_post_index = archillect_local.get_last_post_index()
    input_media_photo_list = []
    for i in range(5):
        ii = archillect_local.get_image(random.randrange(1, last_post_index + 1))
...
def latest_query(message):
    post_id = archillect_local.get_last_post_index()
    ii = archillect_local.get_image(post_id)
    caption = f'[{ii["post_id"]}]({ii["post_url"]})'
    if is_animation(ii['name']):
        bot.send_animation(message.chat.id, ii['url'], caption=caption, parse_mode='MarkdownV2')
...
def manual_id_handler(message):
    ...
    elif int(message.text) > archillect_local.get_last_post_index():
        bot.send_message(message.chat.id, 'entered ID bigger than last post ID')
    else:
        ii = archillect_local.get_image(message.text)
        caption = f'[{ii["post_id"]}]({ii["post_url"]})'
        if is_animation(ii['name']):
            bot.send_animation(message.chat.id, ii['url'], caption=caption, parse_mode='MarkdownV2')
        else:
